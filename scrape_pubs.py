...
def get_pub_deals(client, venue_name, url):
    # LOGIC SWITCH: Route to the exact working prompt variant per venue
    if "Crown" in venue_name:
        ...
        # Dunwoody's exact working prompt structure
        prompt = f"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Use the tailored string cleaning block for each pipeline response style
            if "Crown" in venue_name:
...
try:
    ...
    # Restored clean absolute string URLs to ensure flawless pipeline executions
    venues = [
        {
